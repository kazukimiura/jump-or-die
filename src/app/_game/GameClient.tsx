...
export default function GameClient() {
  ...
  useEffect(() => {
    ...
    /*
     * プレイ領域をブラウザの **トップレイヤー** へ載せる。
     *
     * 目的は「`RUNNING` 中に、ジャンプ以外の反応をする画面領域を一切作らない」
     * （GDD §3-1 禁止事項 / 第4幕の不合格条件3）を**構造的に**満たすこと。
     * レターボックスを含む画面全域がタップ面なので、後から body に差し込まれた
     * 固定要素が 1 枚でも乗れば、そこに落ちたタップはジャンプを発火しない。
     * それは §4-1 の「過検出」と同型の理不尽であり、憲法2・憲法4 に抵触する。
     *
     * CSS の z-index を最大値（2147483647）にしても**保証にはならない**。
     * 同値の場合は後から DOM に挿入された側が上に来るためで、実測でも
     * z-index 最大の後挿入オーバーレイに画面下端のタップを奪われた。
     * `popover="manual"` の要素はトップレイヤーに載り、**z-index の値にも
     * DOM の挿入順にも影響されず**常に最前面になるので、挿入順に依存しない。
     *
     * AdSense Auto ads は社長裁定（2026-09-21）により本ページでは読み込まない
     * （layout.tsx 冒頭参照）。したがって現時点で覆うべき広告は存在しないが、
     * 本措置は広告に限らず**あらゆる後挿入の固定要素**に効くため残している。
     * 実測でトップレイヤーの有無による寸法・位置の差は無い（root / canvas とも同一）。
     *
     * - どの要素も削除・非表示にしない。覆うだけ
