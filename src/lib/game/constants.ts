...
/**
 * `E[D_skill]` の**下限（絶対値）**（GDD §16-12）。
 *
 * 【旧「`D_actual` 目標の 5% 以上」は撤回】
 * 誤りは値ではなく**割合で規定したこと**だった。`D_actual` 目標は S1→S10 で 70倍
 * 伸びるのに `E[D_skill]` は 6倍しか伸びないので、割合で規定すると後半ほど自動的に
 * 下がる。**あの検査は「後半ステージであること」を不合格にしていた。**
 * そもそも `E[D_skill]` はそのまま「習熟後の残存死」を表しており、
 * **`D_actual` との比を取る意味が最初から無い。**
 *
 *     E[D_skill] = 0.5  ⇔  習熟後の通しクリア確率 P = 2/3
 *
 * 根拠は「覚えたら二度と死なない」の防止ではない（それは的外れだった）。
 * **完全に覚えたプレイヤーが毎回クリアできるなら、そのクリアは
 * 「実行できた」ではなく「思い出せた」でしかない。** 本作は1タップの精度を
 * 売りにしたゲームで、最後の1回が実行の勝利でなければジャンルの看板が下りる。
 * **習熟しても3回に1回は落ちる** —— 2/3 が「毎回通る」と「まだ実行を要する」を
 * 分ける自然な線。
 */
export const SKILL_MIN_ABS = 0.5

/**
 * `E[D_skill]` が `D_actual` の目標に占めてよい割合の**上限**（GDD §16-11）。
 * 「覚えたのに運で死ぬ」を防ぐ（**憲法2「覚えれば必ず突破できる」**）。
 * **暫定目標に対しては警告に落とす** —— 仮置きの数字で不合格を出さない。
 */
 ...
 */
/**
 * 章合計の**上限のみ**。下限は `SKILL_MIN_ABS`（絶対値・各本）が担うので持たない。
 * 章に暫定目標が1本でも含まれるときは警告に落とす。
 */
