...
/* ----------------------------------------------------------------------------
 * 位置が動く障害物の運動（G5 `drop` の揺れ / G6 `fly` の上下動）
 *
 * **描画層はこれらのオフセットを自前で足さない。** `RenderObstacle.y` に入ってきた値を
 * そのまま描く。エンジン側が `y` を決める段階で織り込むこと。
 * ここに置いてあるのは「判定と描画が同じ数を使う」ための **共有の純関数**である。
 *
 * どちらも **整数を返す**。丸めをこの 1 箇所に閉じ込めることで、
 * エンジンが float で判定し描画層が round するという分業で生じる
 * 最大 0.5px のずれ（＝ 1px の予兆が 0px に潰れる事故）を構造的に消す。
 *
 * どちらも `Math.sin` を使わない。**整数演算だけで閉じており**、
 * 実行環境による浮動小数の差が入り込む余地が無い（憲法2・決定論）。
 * -------------------------------------------------------------------------- */

/** `drop` の予兆の周期（24f = 400ms） */
export const DROP_WOBBLE_PERIOD = 24
/** `drop` の予兆の振幅（±1px） */
export const DROP_WOBBLE_AMP = 1

/**
 * G5 `drop` の落下前の揺れ。**落ちる個体だけ**に適用する y オフセット。
 *
 * > **【P0】3 値（−1 / 0 / +1）であること。2 値にしてはならない。**
 *
 * 2 値にすると「12f 上・12f 下」のパタパタした矩形波になり、**目立ちすぎて予兆でなくなる。**
 * 3 値なら 1 周期 24f の滞在が **−1: 6f ／ 0: 12f ／ +1: 6f** となり、
 * **基準位置に半分の時間とどまる**ので「パタパタ」ではなく「ゆらぎ」として知覚される。
 *
 * 振幅はスプライト高 16px に対し 1px ＝ 6.25% で**形は一切変わらない**。
 * 周期 400ms は動きとして検出しやすい帯域だが、振幅が小さいので周辺視では捉えられない。
 * 先読み 252px の通過は速度 6.0 で約 1.75 周期 ——
 * **見ようとすれば必ず見えるが、見ていなければ終わる。**
 *
 * 実装は三角波を ±1px へ量子化したものと同値だが、**整数演算だけで閉じている**ため
...
export function dropWobbleOffset(stageFrame: number, phase = 0): number {
  ...
  const t = (((stageFrame + phase) % p) + p) % p
  const q = Math.floor((t * 4) / p) // 0–3
  if (q === 1) return DROP_WOBBLE_AMP
  if (q === 3) return -DROP_WOBBLE_AMP
  return 0
}

/**
 * G6 `fly` の上下動。三角波の y オフセット（整数）。
 *
 * `amp = 0` で常に 0 を返すので、**上下動を持たない既存の飛行体と完全に後方互換**。
 *
 * **羽ばたきと上下動を同期させないこと。** 同期させると「羽ばたきが揚力を生んでいる」という
 * 因果が見え、暗記対象が 1 つ増える。羽ばたきは意味を持たないアイドル動作として
 * `floor(stageFrame / 6) % 2` の **6f 固定で独立**させてある（`drawFlyer` / `drawMouse`）。
 *
 * **`alt` は「基準高度」ではなく「その個体が取りうる最下点」である。**
 * したがって `alt = LOW` ＝ 最下点で地面に接する ＝ 地を走る、は `amp` の値に依らず成立し、
 * `alt=LOW, amp=8` は「跳ねるネズミ」として正しく描ける（描き分けは `alt` 基準のまま）。
 * `amp` 8–24px / `period` 30–90f を想定。`phase` は同一周期の個体を散らすのに使う。
 *
 * 位相は `0 → 下端(−amp) → 中央 → 上端(+amp) → 中央` の三角波で、
 * 正弦波にしないのは**折り返しで速度が落ちる区間を作らない**ため
 * （頂点に留まる動きは「止まって見える」瞬間を作り、先読みを鈍らせる）。
 */
export function flyVerticalOffset(
  stageFrame: number,
  amp: number,
  period: number,
  phase = 0,
): number {
  if (amp === 0 || period <= 0) return 0
  const t = (((stageFrame + phase) % period) + period) % period
  // 三角波を整数のまま作る: 前半は −amp → +amp、後半は +amp → −amp
  const half = period / 2
  const v = t < half ? -amp + (2 * amp * t) / half : 3 * amp - (2 * amp * t) / half
  return Math.round(v)
}

/**
 * G5 `drop`（落ちる浮遊物）。**新規スプライトは無い。`OB_CEILING`（SP-08）を流用する。**
 *
 * 16×16 の GB1 ベタ、**上面 GB3 は描かない**（乗れない・ルールB）。
 * 空中の 16×16 は次の 4 種で一意に決まるので、追加の記号は要らない:
 *
 * | 空中の 16×16 | 上面 GB3 | 面取り | 幅 | 正体 |
 * |---|---|---|---|---|
 * | `lift`  | あり | なし | 16 | 乗れる・縦に動く |
 * | `swing` | あり | あり | 16 | 乗れる・横に動く |
 * | `ceil`  | なし | なし | 48–96（タイル連結） | 乗れない・天井 |
 * | `drop`  | なし | なし | **16（単体）** | 乗れない・落ちうる |
 *
 * **落ちる個体と落ちない個体は同一スプライトで描く。** 静止画で区別できたら
 * 「裏切り」が消え、GDD が案2 を退けた理由をそのまま実装することになる。
 * 情報は画面上に存在する —— ただし静止した形ではなく**揺れという動き**として。
 * 「見えない情報で殺す」のではなく「**見ているのに見落とす**」であり、この 2 つは別物。
 *
 * **落下前・落下中・着地後で同一スプライト。** 変形も回転もしない。
 * **軌跡・残像・着地の衝撃・砂煙は一切描かない**（§9-2 項10 パーティクル禁止）。
 * 着地後も GB1 ＝ 致死のまま。落下開始の合図は「落ち始めること」そのもので、
 * 予告の閃光や色変化は付けない。
 *
 * `crumble` との見分けは**階調だけで足りる** —— `crumble` = GB2（非致死・踏める・上面GB3あり）、
 * `drop` = GB1（致死・乗れない・上面GB3なし）。
 * 「黒いものに触るな」という本作唯一の約束だけで取り違えようがない。
 */
export function drawDrop(
  ...
  drawSprite(ctx, 'CEILING', x, y, hollow)
}
...
export function drawObstacle(
  ...
  switch (ob.kind) {
    ...
    case 'MOUSE':
      ...
      break
    case 'DROP':
      // 揺れ・落下は ob.y に織り込み済み。描画層はオフセットを足さない
      drawDrop(ctx, x, ob.y, hollow)
      break
    case 'SPEAR':
